...
export const ordersService = {
  ...
  rejectOrder: async (orderId: number, reason?: string): Promise<Order> => {
    const response = await api.post(`/orders/orders/${orderId}/reject/`, {
      reason,
    });
    return response.data;
  },
  ...
  markAsPreparing: async (orderId: number): Promise<Order> => {
    const response = await api.post(
      `/orders/orders/${orderId}/mark_preparing/`
    );
    return response.data;
  },
  ...
  markAsShipped: async (
    ...
    const response = await api.post(`/orders/orders/${orderId}/mark_shipped/`, {
  ...
  markAsDelivered: async (orderId: number): Promise<Order> => {
    const response = await api.post(
      `/orders/orders/${orderId}/mark_delivered/`
    );
    return response.data;
  },
