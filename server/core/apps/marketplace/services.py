...
class OpenStreetMapService:
    ...
    def search_location(self, query: str, city: str = None, country: str = "Mauritanie"):
        """Rechercher une location avec OpenStreetMap"""
        try:
            search_query = f"{query}, {city}, {country}" if city else f"{query}, {country}"
            ...
            headers = {
                'User-Agent': 'BetterAgri-Mauritanie/1.0'
            }
    ...
    def get_nearby_places(self, lat: float, lon: float, radius_km: float = 10, place_type: str = None):
                ...
                for element in data.get('elements', []):
                    if 'tags' in element:
                        place = {
                            ...
                            'type': element['tags'].get('shop') or element['tags'].get('amenity', ''),
                            'latitude': element.get('lat') or (element.get('center', {}).get('lat', 0)),
                            'longitude': element.get('lon') or (element.get('center', {}).get('lon', 0)),
    ...
    def get_route(self, start_lat: float, start_lon: float, 
                 end_lat: float, end_lon: float, profile: str = 'driving'):
...
class MauritaniaLocationService:
    ...
    def find_nearby_farmers(self, user_lat: float, user_lon: float, radius_km: float = 50):
