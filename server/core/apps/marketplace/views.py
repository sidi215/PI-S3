...
from .serializers import (
    ...
    ProductReviewSerializer, WishlistSerializer, ProductUpdateSerializer
)
...
class CategoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        main_categories_only = self.request.query_params.get('main_only', 'false').lower() == 'true'
    ...
    def products(self, request, pk=None):
        ...
        ).select_related('farmer').prefetch_related('reviews')
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Product.objects.select_related('farmer', 'category').prefetch_related('reviews')
    ...
    def nearby_products(self, request):
        ...
        products = Product.objects.filter(
            status='active',
            available_quantity__gt=0,
            delivery_radius__gte=0
        ).annotate(
            average_rating=Avg('reviews__rating')
        ).order_by('?')[:20]  # Échantillon aléatoire pour la démo
...
class ProductSearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = Product.objects.filter(
            status='active',
            available_quantity__gt=0
        ).select_related('farmer', 'category').prefetch_related('reviews')
