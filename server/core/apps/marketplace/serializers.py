...
from apps.accounts.serializers import UserSerializer  # Corrigé l'import
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'price_per_unit', 'unit', 'available_quantity', 'harvest_date',
            'expiry_date', 'organic', 'quality_grade', 'farm_location',
            'delivery_radius', 'status', 'main_image', 'images',
            'views_count', 'orders_count', 'is_available',
            'average_rating', 'total_reviews', 'created_at', 'updated_at'
        ]
        read_only_fields = ['farmer', 'views_count', 'orders_count', 'created_at', 'updated_at']
...
class ProductCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Product
        fields = [
            'category', 'name', 'description', 'price_per_unit',
            'unit', 'available_quantity', 'harvest_date',
            'expiry_date', 'organic', 'quality_grade',
            'farm_location', 'delivery_radius', 'main_image'
        ]
    
    def create(self, validated_data):
        request = self.context.get('request')
        validated_data['farmer'] = request.user
        validated_data['status'] = 'active' if validated_data.get('available_quantity', 0) > 0 else 'draft'
...
class ProductUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = [
            'name', 'description', 'price_per_unit', 'unit',
            'available_quantity', 'harvest_date', 'expiry_date',
            'organic', 'quality_grade', 'farm_location',
            'delivery_radius', 'main_image', 'images', 'status'
        ]
    ...
    def update(self, instance, validated_data):
        ...
        if 'available_quantity' in validated_data:
            ...
            elif instance.status == 'sold_out' and validated_data['available_quantity'] > 0:
...
class ProductReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProductReview
        fields = ['id', 'product', 'user', 'rating', 'comment', 'images', 'created_at', 'updated_at']
