    ...
    def update_stats(self, request):
        ...
        if analytics.total_orders > 0:
            analytics.average_order_value = (
                analytics.total_sales / analytics.total_orders
            )
...
class AdminAnalyticsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        total_analytics = FarmerAnalytics.objects.count()
        total_sales = (
            FarmerAnalytics.objects.aggregate(total=Sum("total_sales"))["total"] or 0
        )
...
def update_all_analytics(request):
    ...
    return JsonResponse(
        {"message": f"Loaded analytics for {len(results)} farmers", "results": results}
    )
