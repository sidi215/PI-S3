...
class FarmerWeatherPreferenceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "longitude",
            "receive_rain_alerts",
            "receive_frost_alerts",
            "receive_storm_alerts",
            "receive_drought_alerts",
            "crop_types",
            "planting_date",
            "harvest_date",
...
class WeatherRequestSerializer(serializers.Serializer):
    ...
    latitude = serializers.DecimalField(
        max_digits=9, decimal_places=6, required=True, help_text="Latitude du lieu"
    )
    longitude = serializers.DecimalField(
        max_digits=9, decimal_places=6, required=True, help_text="Longitude du lieu"
    )
...
class AlertNotificationSerializer(serializers.Serializer):
    ...
    action = serializers.ChoiceField(
        choices=["mark_read", "dismiss", "snooze"], default="mark_read"
    )
    snooze_hours = serializers.IntegerField(
        min_value=1, max_value=24, default=4, required=False
    )
