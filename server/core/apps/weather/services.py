...
class MauritaniaWeatherService:
    ...
    def __init__(self):
        ...
        self.base_url = getattr(settings, 'WEATHER_BASE_URL', 'https://api.openweathermap.org/data/2.5')
    ...
    def _get_agricultural_recommendations(self, city_name, weather_data):
        ...
        if temp > 35:
            recommendations.append("🌡️ Températures élevées : Arroser tôt le matin ou en soirée")
        
        if temp < 15:
            recommendations.append("❄️ Températures fraîches : Protéger les cultures sensibles")
        
        if precipitation < 2 and temp > 30:
            recommendations.append("💧 Faibles précipitations : Augmenter la fréquence d'irrigation")
        
        if humidity < 20:
            recommendations.append("🌵 Humidité très basse : Utiliser l'irrigation goutte-à-goutte")
        
        if wind_speed > 20:
            recommendations.append("💨 Vents forts : Sécuriser les serres et équipements")
        ...
        if zone == 'Côtier':
            ...
            recommendations.append("🏜️ Zone désertique : Privilégier les cultures résistantes à la sécheresse")
        elif zone == 'Soudanien':
            recommendations.append("🌾 Zone soudanienne : Bonne période pour les céréales")
    ...
    def _parse_api_response(self, api_data, city_name):
        ...
        return {
            ...
            'precipitation': api_data.get('rain', {}).get('1h', 0) or api_data.get('snow', {}).get('1h', 0),
        }
...
class AIService:
    ...
    def generate_crop_suggestions(self, weather_data):
        ...
        if 20 <= temp <= 35 and humidity >= 30:
            suggestions.extend([
                {'crop': 'Tomates', 'season': 'Toute l\'année', 'notes': 'Bonne résistance'},
                {'crop': 'Concombres', 'season': 'Printemps-Été', 'notes': 'Nécessite un arrosage régulier'},
                {'crop': 'Poivrons', 'season': 'Printemps-Été', 'notes': 'Sensible au froid'},
            ])
        
        if temp > 30 and humidity < 30:
            suggestions.extend([
                {'crop': 'Dattes', 'season': 'Annuelle', 'notes': 'Très résistant à la sécheresse'},
                {'crop': 'Melons', 'season': 'Été', 'notes': 'Nécessite beaucoup d\'eau'},
                {'crop': 'Pastèques', 'season': 'Été', 'notes': 'Adapté aux climats chauds'},
            ])
        
        if temp < 20:
            suggestions.extend([
                {'crop': 'Laitue', 'season': 'Hiver-Printemps', 'notes': 'Culture rapide'},
                {'crop': 'Carottes', 'season': 'Automne-Hiver', 'notes': 'Sol bien drainé'},
                {'crop': 'Oignons', 'season': 'Hiver', 'notes': 'Facile à cultiver'},
            ])
