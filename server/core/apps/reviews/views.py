...
class FarmerReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.user_type == "farmer":
            ...
            return FarmerReview.objects.filter(farmer=user).select_related(
                "reviewer", "order"
            )
        else:
            # Un acheteur voit les avis qu'il a donnés
            return FarmerReview.objects.filter(reviewer=user).select_related(
                "farmer", "order"
            )
    ...
    def farmer_stats(self, request):
        ...
        # Détails des avis
        rating_distribution = (
            FarmerReview.objects.filter(farmer=farmer)
            .values("rating")
        ...
        return Response(
            {
                ...
                "verified_reviews": FarmerReview.objects.filter(
                    farmer=farmer, is_verified=True
                ).count(),
            }
        )
    ...
    def by_farmer(self, request, farmer_id=None):
        ...
        reviews = FarmerReview.objects.filter(farmer=farmer).select_related(
            "reviewer", "order"
        )
...
class RecentReviewsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return (
            FarmerReview.objects.filter(is_verified=True)
            .select_related("farmer", "reviewer")
