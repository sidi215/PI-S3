...
class CreateFarmerReviewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if self.context["request"].user.user_type != "buyer":
            raise serializers.ValidationError("Only buyers can submit reviews")
        ...
        if order.buyer != self.context["request"].user:
            raise serializers.ValidationError("You can only review orders you placed")
        ...
        if FarmerReview.objects.filter(
            farmer=data["farmer"], reviewer=self.context["request"].user, order=order
        ).exists():
